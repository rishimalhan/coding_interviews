...
labels_file = pd.read_csv(data_path+"/labels.txt",sep=' ',header=None)
labels = np.column_stack( [labels_file[1],labels_file[2]] )
results = np.array([])
...
for path in pathlist:
	...
	if not N:
		...
		cX = int(M["m10"] / M["m00"])
		cY = int(M["m01"] / M["m00"])
		results = np.append(results, [float(cX)/image.shape[1], float(cY)/image.shape[0]] )
...
print( np.linalg.norm( results-labels,axis=0 ) )